...
if(classicTF=="classic"){
    ar.loadings.2tcb = apply(ar.mean.rtdb[,,,],2:4,quantile,prob=c(.025,.975))
}else{
    ar.loadings.2tcb = array(NA,dim=c(2,n.type,2,n.boot),
                             dimnames=list(c("low","high"),id.type$id,paste0("Dim",1:2),id.boot$id))
    for(bw in 1:n.boot){
        ar.loadings.2tcb[,,,bw] = apply(ar.mean.rtdb[notoutliers.r_boot[[bw]],,,bw],
                                        2:3,quantile,prob=c(.025,.975))
        }
}
# original estimates
...
for(tw in 1:n.type){
    for(bw in 1:n.boot){
        if(classicTF){
            posw = rep(TRUE,length(notoutliers.r_boot[[bw]]))
        }else{
            posw = notoutliers.r_boot[[bw]]
        }
        points(rep(tw+c(-.1,0,.1)[bw],sum(posw)),                    
                         ar.mean.rtdb[posw,tw,dimw,bw],
    ...
    for(i in 1:n.type){
        for(j in i:n.type){
            ...
            # off-diagonal
                ...
                if(classicTF){
                    posw = rep(TRUE,length(notoutliers.r_boot[[bootw]]))
                }else{
                    posw = notoutliers.r_boot[[bootw]]
                }
                loadings_i = ar.mean.rtdb[posw,i,dw,bootw]
                loadings_j = ar.mean.rtdb[posw,j,dw,bootw]
